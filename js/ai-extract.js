...
export function openAIExtractSheet() {
  ...
  const { root, close } = openSheet(`
    ...
    <div class="sheet-content ai-sheet">
      ...
      <textarea id="aiInput" class="input" rows="6" placeholder="输入你的任务文本，例如：重要事项有开会、写报告，杂事有买菜、回消息…"></textarea>
  ...
  root.querySelector('#runExtract').addEventListener('click', async () => {
    ...
    try {
      ...
      renderResult(root, parsed, boxes);
    } catch {
...
function renderResult(root, parsed, boxes) {
  const rows = [];
  ...
  const render = () => {
    ...
    wrap.querySelectorAll('[data-i]').forEach((el) => el.addEventListener('change', () => { rows[el.dataset.i].checked = el.checked; render(); }));
    wrap.querySelectorAll('[data-c]').forEach((el) => el.addEventListener('input', () => { rows[el.dataset.c].content = el.value; }));
    wrap.querySelectorAll('[data-b]').forEach((el) => el.addEventListener('change', () => { rows[el.dataset.b].boxId = el.value; }));
    wrap.querySelectorAll('[data-del]').forEach((el) => el.addEventListener('click', () => { rows.splice(el.dataset.del, 1); render(); }));

    wrap.querySelector('#confirmAdd')?.addEventListener('click', () => {
      rows.filter((r) => r.checked && r.content.trim()).forEach((r) => addTask({ boxId: r.boxId, content: r.content.trim() }));
      showToast('任务已添加');
    });
  };
