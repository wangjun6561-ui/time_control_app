...
const DEFAULT_BOXES = [
  ...
  { name: '碎片学习盒', color: 'misc', icon: '🧩', sortOrder: 5, isDefault: true, description: '碎片时间学习清单，想到就看。' },
];
...
function normalize(data = {}) {
  return {
    boxes: (Array.isArray(data.boxes) ? data.boxes : []).map((b) => {
      ...
      const orderMap = { '重要盒': 0, '放松盒': 1, '奖励盒': 2, '待办盒': 3, '惩罚盒': 4, '碎片学习盒': 5 };
...
export function getTasksByBox(boxId) {
  return getTasks()
    ...
    .sort((a, b) => (Number(b.weight)||1) - (Number(a.weight)||1) || (Number(b.priority)||0) - (Number(a.priority)||0) || a.sortOrder - b.sortOrder || new Date(a.createdAt) - new Date(b.createdAt));
}
