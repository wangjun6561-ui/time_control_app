...
function openTaskEditor({ taskId, boxId }, onDone) {
  ...
  const { root, close } = openSheet(`
    ...
    <div class="sheet-content">
      ...
      <label>感悟（可选）<textarea id="taskReflection" class="input" rows="2">${task?.reflection || ''}</textarea></label>
      <label>复盘（可选）<textarea id="taskReview" class="input" rows="2">${task?.review || ''}</textarea></label>
      <label>总结（可选）<textarea id="taskSummary" class="input" rows="2">${task?.summaryText || ''}</textarea></label>
  ...
  root.querySelector('#saveBtn').addEventListener('click', () => {
    ...
    const payload = {
      ...
      boxId: nextBoxId,
      reflection: root.querySelector('#taskReflection').value.trim(),
      review: root.querySelector('#taskReview').value.trim(),
      summaryText: root.querySelector('#taskSummary').value.trim(),
      isCompleted: done,
